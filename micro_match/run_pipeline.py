...
def microMatch_pipeline():
    cur_dir = "/Users/lfranz/Desktop/tracking/mumesh/all_follicles/"
    collection_ids = ["170517", "170519"]
    dataset_id = "FollicleDataset"

    cur_dir = os.path.join(os.getcwd(), "example_data")
    """
    ## Parameter file
    ...
    * data_dir should point to where we want the processed data to be stored for later use.
    ...
    collection_0 = [file_name for file_name in raw_files if collection_ids[0] in file_name]
    collection_1 = [file_name for file_name in raw_files if collection_ids[1] in file_name]
    data_dir = os.path.join(cur_dir, "processed_data")
    ...
    if use_deep_learning:
        ...
        optimise_signatures.process_directory(data_dir=data_dir, config=config["preprocessing"], mesh_type=dataset_id)
    ...
    * A more advanced way is to compute all pairwise correspondences. This will take much longer, but then improvement schemes can be used subsequently to increase the qualtity of the correspondences.
    ...
    match_dir = os.path.join(cur_dir, "match_results")
    if not os.path.exists(match_dir):
        os.makedirs(match_dir)

    matching_functional = Match(dir_in=data_dir, dir_out=match_dir, config=config["correspondence"], display_result=False)
    import pandas as pd
    geodesic_distortions = pd.DataFrame(0, index=collection_0, columns=collection_1)
    for follicle in collection_0:
        for n_follicle in collection_1:
            geodesic_distortions.loc[follicle, n_follicle] = matching_functional(follicle, n_follicle)
    geodesic_distortions.to_csv(os.path.join(match_dir, "geodesic_distortions.csv"))
