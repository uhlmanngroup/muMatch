...
def filter_histogram(x):
    ...
    x = ndimage.convolve(x, np.expand_dims(tri, axis=(1,2,3)), mode = 'nearest')
    x = ndimage.convolve(x, np.expand_dims(tri, axis=(0,2,3)), mode = 'wrap')
    x = ndimage.convolve(x, np.expand_dims(tri, axis=(0,1,3)), mode = 'nearest')
    x = ndimage.convolve(x, np.expand_dims(tri, axis=(0,1,2)), mode = 'nearest')
