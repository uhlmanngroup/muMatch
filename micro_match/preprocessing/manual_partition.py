...
class partition_viewer(vp.plotter.Plotter):
    ...
    def custom_update(self):
        ...
        if (0 < len(self.__left)):
            self.__spheres.append(vp.shapes.Spheres( self.__left, r = .04, c = 'r'))
        if (0 < len(self.__right)):
            self.__spheres.append(vp.shapes.Spheres(self.__right, r = .04, c = 'g'))
    ...
    def partition_mesh(self):
        ...
        left  =  [np.argmin(np.linalg.norm(self.__pts - l, axis = -1)) for l in self.__left]
        right =  [np.argmin(np.linalg.norm(self.__pts - r, axis = -1)) for r in self.__right]
        N     =  self.__g.shape[0]
