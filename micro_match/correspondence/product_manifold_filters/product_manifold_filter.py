...
def product_manifold_filter_correspondence(
    assignment_functor, gx, gy, i, j, config
):
    sigma, gamma, iterations = (
        config[key] for key in ["sigma", "gamma", "iterations"]
    )
...
def product_manifold_filter_assignment(assignment_functor, gx, gy, P, config):
    i, j = assignment_functor(P)
    return product_manifold_filter_correspondence(
        assignment_functor, gx, gy, i, j, config
    )
