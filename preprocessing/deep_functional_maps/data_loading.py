...
def generate_TFRecord(dir_in, N, output_name):
    ...
    tfrecords_filename = os.path.join(cur_dir, "data", "{}.tfrecords".format(output_name))
    writer = tf.io.TFRecordWriter(tfrecords_filename)
    names = [os.path.splitext(f)[0] for f in os.listdir(os.path.join(dir_in, "signatures"))]

    for fn in names:
        #try:
        s = np.load(os.path.join(dir_in, 'signatures', fn + '.npy')).astype(np.float32)
        eigen = np.load(os.path.join(dir_in, 'eigen', fn + '.npz'))
        e,e_t = [eigen[k].astype(np.float32) for k in ["evecs", "evecs_t"]]
        g   =  np.load(os.path.join(dir_in, 'geodesic_matrices', fn + '.npy')).astype(np.float32)
