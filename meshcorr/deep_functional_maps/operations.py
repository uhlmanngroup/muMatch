...
class correspondenceMatrix(tf.keras.Model):

    def __init__(self, num_descriptors, training):
        super(correspondenceMatrix, self).__init__()
        self.resnet  = residualNet(7, num_descriptors, training)

    def __call__(self, sigs, evecs_t):
        sigs  = [self.resnet(x) for x in sigs]
